...
#   CHART_REF              (default: latest local rendered SUSE chart under charts/)
#   CHART_VERSION          (default: "" — omitted when using a local path)
#   IMAGE_LOGGING_OPERATOR (default: ghcr.io/manno/logging-operator:dev-775aefe4)
#   IMAGE_CONFIG_RELOADER  (default: ghcr.io/manno/config-reloader:dev-54f3a9a)
#   IMAGE_FLUENT_BIT       (default: ghcr.io/manno/fluent-bit:dev-4a98803c1)
...
#
#   CREATE_CRD=false       skip CRD install when Rancher has pre-installed them
...
: "${CHART_REF:=$(ls -d "$(dirname "$0")/../charts/rancher-logging/4.10.0-rancher."*"-suse1" 2>/dev/null | sort -V | tail -1)}"
: "${CHART_VERSION:=}"
: "${IMAGE_LOGGING_OPERATOR:=ghcr.io/manno/logging-operator:dev-775aefe4}"
: "${IMAGE_CONFIG_RELOADER:=ghcr.io/manno/config-reloader:dev-54f3a9a}"
: "${IMAGE_FLUENT_BIT:=ghcr.io/manno/fluent-bit:dev-4a98803c1}"
...
if [ -z "$CHART_REF" ]; then
  echo "ERROR: no rendered SUSE chart found under charts/rancher-logging/. Run 'make render' first." >&2
  exit 1
fi
...
HELM_ARGS=(
  ...
  --set "images.config_reloader.repository=$CR_REPO"
  --set "images.config_reloader.tag=$CR_TAG"
  --set "images.fluentbit.repository=$FB_REPO"
  --set "images.fluentbit.tag=$FB_TAG"
  --set "images.fluentbit_debug.repository=$FB_REPO"
  --set "images.fluentbit_debug.tag=$FB_TAG"
  --set "images.fluentd.repository=$FD_REPO"
  --set "images.fluentd.tag=$FD_TAG"
  --set "createCustomResource=${CREATE_CRD}"
...
# Rancher chart uses name=rancher-logging; upstream chart uses name=logging-operator
OPERATOR_DEPLOY=$(kubectl -n "$NAMESPACE" get deploy \
  -l app.kubernetes.io/name=logging-operator -o name 2>/dev/null | head -1)
[ -n "$OPERATOR_DEPLOY" ] || OPERATOR_DEPLOY=$(kubectl -n "$NAMESPACE" get deploy \
  -l app.kubernetes.io/name=rancher-logging -o name 2>/dev/null | head -1)
...
log "Applying ClusterOutput + ClusterFlow that pipes test pod logs to a file in fluentd"
...
log "Searching fluentd output file for sentinel (timeout $TIMEOUT_LOG_FLOW)"
...
while [ "$(date +%s)" -lt "$deadline" ]; do
  if kubectl -n "$NAMESPACE" exec "$FD_POD" -- grep -q "$SENTINEL" /tmp/smoke-test.log 2>/dev/null; then
    ok "Sentinel '$SENTINEL' found in fluentd output file -- end-to-end flow works"
...
fail "Sentinel '$SENTINEL' did NOT appear in /tmp/smoke-test.log within $TIMEOUT_LOG_FLOW"
log "Last 20 lines of /tmp/smoke-test.log:"
kubectl -n "$NAMESPACE" exec "$FD_POD" -- tail -20 /tmp/smoke-test.log 2>/dev/null || true
log "Last 20 lines of fluentd pod log:"
kubectl -n "$NAMESPACE" logs "$FD_POD" --tail=20 || true
exit 1
